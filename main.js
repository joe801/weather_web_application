...
function getlocation(){
    ...
    //fetch statement to get api key from token.txt file
    fetch('token.txt')
    .then((res) => res.text())
    .then((apikey) => {
        console.log(apikey)
        ...
        .then((data) => {
            ...
            }) //catch error for error in fetching weather
            .error((error) => {
                console.error(error);
            })
        }) //catch error for error in fetching longitude and lattitude
        .catch((error) => {
            console.error(error);
        })
    }) //catch error for error in fetching api key
    .catch((error) => {
        console.error(error);
    });
}